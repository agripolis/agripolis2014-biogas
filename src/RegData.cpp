...
void RegDataInfo::printRegionSoilservice(ofstream &c_ofs, list <RegFarmInfo*> farmList, int period) {
	...
	c_ofs << "###########   \n";
	c_ofs << setw(20) << period << setw(20)<< 999999 << setw(20)<< "whole_region" << setw(20);
	for (int i=0; i<g->NO_OF_SOIL_TYPES; ++i) {
		c_ofs << avC[i] << setw(20) << varC[i] << setw(20);
	}
	c_ofs << "\n###########    \n";
...
void RegDataInfo::initSoilservice(list <RegFarmInfo*> farmList){
	...
	c_ofs<<setw(20);
	c_ofs<<"iteration"<<setw(20) << "farm_id" <<setw(20)<< "farm_name"<<setw(20);
	for (int j=0; j< g->NO_OF_SOIL_TYPES; ++j) {
		c_ofs << "mean_"+ g->NAMES_OF_SOIL_TYPES[j]<<setw(20)<<"var_"+g->NAMES_OF_SOIL_TYPES[j]<<setw(20);
	}
	...
	for (int i= 0; i<nprod; ++i) {
		if (prodcat[i].hasDynSoilservice()){
			c_ofs << prodcat[i].getName()+"_N" << setw(20)<< prodcat[i].getName()+"_Yield" << setw(20);
		}
	}
	...
	for (iter = farmList.begin(); iter!=farmList.end();++iter) {
		...
		for (int i=0; i<g->NO_OF_SOIL_TYPES; ++i) {
			if (erst) {
				c_ofs<<setw(20) << "-1" <<setw(20)<< (*iter)->getFarmId() <<setw(20) << (*iter)->getFarmName() <<setw(20);
				erst=false;
			}
			c_ofs << (*iter)->getAvCarbons()[i]<<setw(20)<< (*iter)->getVarCarbons()[i] <<setw(20);
		}
		for (int i= 0; i<nprod; ++i) {
			if (prodcat[i].hasDynSoilservice()){
				c_ofs<<(*iter)->getProductList()->getNoptOfNumber(i)<<setw(20) << (*iter)->getProductList()->getYieldOfNumber(i,*iter) << setw(20);
			}
		}
		c_ofs << "\n"<<setw(20);
	}
...
void RegDataInfo::printSoilservice(list <RegFarmInfo*> farmList, int period){
	...
	for (iter = farmList.begin(); iter!=farmList.end();++iter) {
		...
		for (int i=0; i<g->NO_OF_SOIL_TYPES; ++i) {
			if (erst) {
				c_ofs<<setw(20) << period <<setw(20)<< (*iter)->getFarmId() <<setw(20) << (*iter)->getFarmName() <<setw(20);
				erst=false;
			}
...
RegDataInfo::printSectorPrices( const RegSectorResultsInfo& sector,
    ...
    secpricesout << setw(11) << (int)g->SCENARIO << "\t"
    << setw(11) << (double)g->V << "\t"
...
RegDataInfo::printExpectedSectorPrices( const RegSectorResultsInfo& sector,
    ...
    secpricesout.open(file.c_str(), ios::app);
    secpricesout << setw(11) << (double)period  << "\t";
    for (unsigned int i = 0; i < product_cat.size(); i++) {
        secpricesout << setw(11) << product_cat[i].getPriceExpectation() << "\t";
//         secpricesout << setw(11) << product_cat[i].getPrice() << "\t";
    }
...
RegDataInfo::printSectorResults(const RegSectorResultsInfo& sector,
    ...
    // SECTOR RESULTS
    sector_values.push_back( (int)g->SCENARIO                         );
 ...
 //DCX
    sector_values.push_back(g->MANSELL < (int)product_cat.size() ? product_cat[g->MANSELL].getPriceExpectation() :0 );
    sector_values.push_back(g->MANBUY< (int)product_cat.size() ? product_cat[g->MANBUY].getPriceExpectation() :0  );
    for (int i=0;i<g->NO_OF_SOIL_TYPES;i++) {
...
RegDataInfo::printFarmProduction(const RegFarmInfo* farm,
    ...
    farmprodout << setw(11) << (int)g->SCENARIO << "\t"
    << setw(11) << (double)g->V << "\t"
    << setw(11) << (double)period  << "\t"
    << setw(11) << (double)farm->getFarmId()            << "\t"
...
RegDataInfo::printFarmInvestment(const RegFarmInfo* farm,
    ...
    // FARM DATA
    farminvest  << setw(11) << (int)g->SCENARIO << "\t"
    << setw(11) << (double)g->V << "\t"
    << setw(11) << (double)period  << "\t"
    << setw(11) << (double)farm->getFarmId()            << "\t"
    << setw(11) << (farm->getFarmName()).c_str()            << "\t"
...
//    res.push_back(farm->getFarmName().c_str()          );
    ...
    // SUBSIDIES
	if(g->LP_MOD) {
    	res.push_back(farm->getUnitsOfProduct(g->COUPLED_PREM_UNMOD) );
    	res.push_back(farm->getUnitsOfProduct(g->DECOUPLED_PREM_UNMOD) );
    	res.push_back(farm->getUnitsOfProduct(g->TOTAL_PREM_MODULATED) );
	} else {
    	res.push_back(farm->getModulatedIncomePaymentFarm() );
    	res.push_back(farm->getUnitsOfProduct(g->PREMIUM) );
	}
