...
void RegFarmInfo::initCarbons(vector<double> mean, vector<double> var) { //var --std_dev
	...
	//Achtung ! avCabons wird benutzt, auch wenn 0 plots für diesen Bodentyp
...
//DCX
    ...
    // colour
/*    int R = (int)(((double)rand()/RAND_MAX)*5);
    int GR = (int)(((double)rand()/RAND_MAX)*5);
    int B = (int)(((double)rand()/RAND_MAX)*5);
...
//*/
    ...
    if (g->USE_TRIANGULAR_DISTRIBUTED_MANAGEMENT_FACTOR) {
        management_coefficient=triangular(lower_border,lower_border+(upper_border-lower_border)/2,upper_border);
    } else {
    ...
    //updateLpValues();  nicht nötig --dcx
   ...
   /* 	
    switch (farm_class) {
    case 1: // pig and poultry
        farm_plot = region->getRandomFreePlotOfType(0);
        break;
    case 2: // dairy
        farm_plot = region->getRandomFreePlotOfType(0);
        break;
...
RegFarmInfo::setAsynchronousFarmAge() {
    ...
    if (g->USE_TRIANGULAR_DISTRIBUTED_FARM_AGE) {
        ran = static_cast<int>( triangular(1,12,g->GENERATION_CHANGE));
    } else {
...
//    wanted_plot = farm_plot->findFreePlotOfType(type);
    ...
    if (wanted_plot != NULL) {
        ...
        int cl=1+randlong()%(g->MAX_CONTRACT_LENGTH-1);
...
RegFarmInfo::doLpInvest() {
    ...
    // determine the catalog number of investment and the number of
    // investments
    ...
    //  nur, wenn TF gleich wirksam ist und nicht erst zu Beginn der naechsten
    //  Periode
    if (invested)
        adjustVarCosts();
    calculateLiquidity();
...
RegFarmInfo::periodResults(int period) {
    ...
    if(g->LP_MOD) {
	modulated_income_payment=getUnitsOfProduct(g->TOTAL_PREM_MODULATED);
    display_modulation=0;
    if((int)(getUnitsOfProduct(g->TRANCH_1)+0.5)>0)
      display_modulation=1;
    if((int)(getUnitsOfProduct(g->TRANCH_2)+0.5)>0)
      display_modulation=2;
    if((int)(getUnitsOfProduct(g->TRANCH_3)+0.5)>0)
      display_modulation=3;
    if((int)(getUnitsOfProduct(g->TRANCH_4)+0.5)>0)
      display_modulation=4;
    if((int)(getUnitsOfProduct(g->TRANCH_5)+0.5)>0)
    ...
    // GESAMTEINKOMMEN zur Berechnung der Eigenkapitalveraenderung
    ...
    // labour yield (ARBEITSERTRAG)
    // = profit - interest on average bound equity capital
    // = Gewinn - Zinsansatz fuer durchschnittlich gebundenes EK
...
RegFarmInfo::periodResultsForRemovedFarms() {
    ...
    oppcostslabourhour = (- FarmInvestList->getAcquisitionCostsOfNumber(g->FIXED_OFFFARM_LAB)) /
                         - FarmInvestList->getLabourSubstitutionOfNumber(g->FIXED_OFFFARM_LAB);
    double quotarent = - milk_quota * (*product_cat)[g->GETQUOTA].getPriceExpectation();
    // farm ages one year
    farm_age++;
    interest_costs = 0;
    ...
    // Interest: Die Tilgung der einzelnen Investitionsobjekte wird in RegInvestList::depreciateCapital
    // vorgenommen. Deshalb müssen die Zinsen vorher betrechnet werden.
...
RegFarmInfo::futureOfFarm( int period) {
    ...
    quotarent = - milk_quota * (*product_cat)[g->GETQUOTA].getPriceExpectation();

    // change of generation every x years
    // hoc = high oppportunity costs
    int hoc = farm_age%g->GENERATION_CHANGE;
    // DECISION ON FUTURE OF FARM

    if ((legal_type==1) && (no_successor_by_random) && (hoc==0)) {
...
// gibt false zurück falls deltayield>0 oder kein Plot des Typs mehr vorhanden ist
// sonst true ; type 0:arable 1:grass
bool
...
RegFarmInfo::getCapitalInputDEA() const {
    ...
    double services = 0;
    double quota = 0;
    services = (-1)* FarmProductList->getGrossMarginOfNumber(g->SERVICES)*FarmProductList->getUnitsProducedOfNumber(g->SERVICES);
    quota = (getUnitsOfProduct(g->MILK)*g->MILKPROD)* FarmProductList->getPriceOfNumber(g->LETQUOTA);
...
RegFarmInfo::getOutputRevenue() const {
    ...
    revservices = FarmProductList->getReturnOfNumber(g->SERVICES);
    revletquota = FarmProductList->getReturnOfNumber(g->LETQUOTA);
    revgetquota = FarmProductList->getReturnOfNumber(g->GETQUOTA);
...
RegFarmInfo::getOutputGrossMarginDEA() const {
    ...
    gmservices = FarmProductList->getGrossMarginOfNumber(g->SERVICES)*FarmProductList->getUnitsProducedOfNumber(g->SERVICES);
    gmletquota = FarmProductList->getGrossMarginOfNumber(g->LETQUOTA)*FarmProductList->getUnitsProducedOfNumber(g->LETQUOTA);
    gmgetquota = FarmProductList->getGrossMarginOfNumber(g->GETQUOTA)*FarmProductList->getUnitsProducedOfNumber(g->GETQUOTA);
...
RegFarmInfo::getEconomicLandRent()  const {
    ...
    double ecinterest = ((equity_capital-land_assets-ecchange)*g->EQ_INTEREST)
                        /land_input;
    double quota = ((FarmProductList->getUnitsProducedOfNumber(g->MILK)*g->MILKPROD
                     + FarmProductList->getUnitsProducedOfNumber(g->LETQUOTA)
                     - FarmProductList->getUnitsProducedOfNumber(g->GETQUOTA)
