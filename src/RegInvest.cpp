...
RegInvestList::removeInvestment() {
    ...
    for (invest = farm_invests.begin();
        ...
        if ((*invest).getEconomicLife() <= (*invest).getInvestAge()) {
            // investment is dead -1 // delete from list or move to back of list
...
//        if ((*invest).getInvestAge()==0) Application->MessageBox("Invest Panic!", "Message", MB_OK);
        //
        ...
        if ( (sunkcostrelevant == true) && ((*invest).getAcquisitionCosts() > 0) ) {
            ...
            // ResidualValue and ResidualEcShare
            // Irgendwie sehe ich da gerade nicht ganz durch, deshalb verwende ich die Formel aus setAsynchronusInvestAge
            // Bis ich mir das jetzt klar mache komme ich sonst zu nciht mehr viel! Eigentlich koennte ich doch die Abschreibung einfach
            // abziehen aber ich habe ja noch gar keinen Ausgangswert da dieser ja nicht gesetzt wird
            ...
            // alt und ungenau!!!!
            //double bc_share = (1 - v) * residualvalue;
        ...
        if ( (sunkcostrelevant == true) && ((*invest).getAcquisitionCosts() < 0) ) {
            ...
            // labincome braucht hier eigentlich nicht berechnet zu werden, da
            // man es ueber die Investitionsliste abfragen kann
            // Da es jeweils nur einjaehrige kontrakte sind, ist die Verzinsung = 1
            labincome += ( acqcosts *
                           (v * pow((1 + eq_interest), investage - getolder) *
                            (capitalReturnFactor(eq_interest, n) - eq_interest) +
                            (1 - v) * pow((1 + bc_interest), investage - getolder) *
            ...
            // value added braucht man hier ebenfalls nicht zu berechnen, da spaeter
            // retrograd ermittelt wird
            *value_added += ( acqcosts *
                              (v  * pow((1 + eq_interest), investage - getolder) *
                               (capitalReturnFactor(eq_interest, n) - eq_interest) +
                               (1 - v) * pow((1 + bc_interest), investage - getolder) *
    ...
    for (invest = farm_invests.begin();
        ...
        *assets += acqcosts; // assets for full lifetime of investment
        ...
        if ((investage > 0)&&(acqcosts > 0)) {
            ...
            // nach Alfons Formel vom 15.03.2001

            // BUG!!!! HIER MUSS DAS -1 raus im Ausdruck (n-(investage-1))!!!!!
