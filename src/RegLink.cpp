...
RegLinkYieldObject::trigger() {
	...
	switch (value_kind) {
    case 0:  res_value=res_value*factor*1000; //???????
			break;
	case 1:  res_value *=-t->getPOfNumber(source_number)*factor;
			break;
	case 2: res_value *=-t->getKOfNumber(source_number)*factor;
			break;
	case 3: res_value *=-t->getPesticideOfNumber(source_number)*factor;
			break;
