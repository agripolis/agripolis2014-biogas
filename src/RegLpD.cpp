...
//static    map <string, int> marketind;
...
/*    int ind=0;
    for (map <string,double>::iterator it=matlinkdata.mark_fac.begin();
        it != matlinkdata.mark_fac.end();it++) {
            marketind[(*it).first]=ind++;
    }
//*/
...
/*    // activities after prodcols are integer activities
    for (int i = 0; i < numcols; i++) {
        // +2 are the 2 livestock restrictions
        if (i < prodcols  ) { // prodcols + 2 = 26   || i != 39  הההההההה?
            ctype[i]='C';
        }
        if (i >= prodcols ) {             // || i == 39 הההההההההההההה
            ctype[i]='I';
        }
    }
//*/
    ...
    for (int i = 0; i < numcols; i++) {
        lb[i] = 0.0;
        if (i < prodcols) { // prodcols = 25
            ub[i] = +1E30;
           // ub[g->EXCESS_LU] = 0;
            //     ub[g->IDLE_ARABLE] = 0;//INFBOUND;
            //      ub[g->IDLE_GRASS] = 0;
            //ub[g->LU_UPPER_LIMIT] = 0;
        }
        /*                    if(i==prodcols-5){  // 24 income payment
                                ub[i] = 0;//0;
                            }
        */
        if (i >= prodcols) { // ab 26
...
//DCX
     //       PList->setUnitsProducedOfNumber(s,x[d]);
        ...
        case 3:
            break;
        default:
            // Aus Debug gruenden wird default ein unsinniger Wert gesetzt
            if (d< prodcols) {
...
//*/
	...
	iparm.mir_cuts=GLP_OFF; //GLP_ON Arlette 24.10.2012
