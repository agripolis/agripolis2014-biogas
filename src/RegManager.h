...
public:
    ...
    /** Initialisation of all major classes and reading of input data. After
        this method the programme is ready to run. Calls
        \begin{itemize}
        \item    initRegion();
        \item    initInvestmentCatalog();
        \item    initMarket();
        \item    initPopulations();
        \item    initMatrix();
        \item    initOutput();
        \item    initGlobals();
        \end{itemize}
...
protected:
    ...
    int bidcount;
    /// method controls the programme sequence
    /// pointer to globals
    RegGlobalsInfo* g;
...
public:
    ...
    string col;
    int row;
    void readPosition(string n) {
        string temp;
        xl->gotoCellWithString(n);
        temp=xl->readNextStringValue();
        ...
        if (temp=="NOT") {
            ...
            exists=true;
            col=temp;
            row=xl->readNextIntValue();
        }
    }

    void readValue() {
        if (exists)
           ; //value.push_back(xl->readDoubleValueAt(col,row));
    }
};
//*/
