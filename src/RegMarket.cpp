...
RegMarketInfo::createMarket(RegEnvInfo* Env) {
    ...
    for (int i=0;i<num_products;i++) {
        ...
        product_cat[counter].setAttrib(i,
                ...
                marketdata.products[i].name,marketdata.products[i].prodbytype);
...
//            product_cat[counter].setPremiumRow(xl->readIntValueAt(premium_row,i));
    ...
    if (g->ENV_MODELING) {
        int counter=0;
        for (int i=0; i< num_products; i++) {
 /*             double t =      envmarketdata.envproducts[i].n;
              double t1 =           envmarketdata.envproducts[i].p2o5;
              double t2 =          envmarketdata.envproducts[i].k2o;
              double t3 =           envmarketdata.envproducts[i].fungicide;
              double t4 =          envmarketdata.envproducts[i].herbicide;
              double t5 =    envmarketdata.envproducts[i].insecticide;
              double t6 =          envmarketdata.envproducts[i].water;
              double t7 =       envmarketdata.envproducts[i].soil_loss;
                 int ti =          envmarketdata.envproducts[i].biohabitat;
   //*/
	...
	if (g->HAS_SOILSERVICE) {
		...
		for (int i=0; i<sz; ++i) {
			getProductCat()[prodId[yielddata[i].name]].setSoilservice(yielddata[i].a, yielddata[i].b, 
					yielddata[i].c, yielddata[i].d, yielddata[i].gamma, soilId[yielddata[i].soiltype], true, yielddata[i].dyn,
...
RegMarketInfo::calculatePricevectors(RegSectorResultsInfo& Sector, int iteration){
	...
	vector <double> old;            // ####changed 
	old.resize(product_cat.size());  //###
    ...
    for (int i=0;i < (int)product_cat.size();i++) {
      if(g->FIX_PRICES) {
        ...
        if (i==g->SOW) { // sow production
            prod[i] = (unitsproduced - Sector.getTotalUnitsProduced(g->FATTENINGPIGS)*g->PIGS_PER_PLACE/g->PIGLETS_PER_SOW);
            if (prod[i] <= 0)
                prod[i]=1;
        }  else {
            prod[i]/=totallandha;
        }
        // ALL PRODUCTS other than MANSELL, MANBUY, etc. 
        ...
        // MANURE MARKET
        if (i==g->MANSELL || i==g->MANBUY) {
            double totalmanuresold = g->MANSELL < (int)product_cat.size() ? Sector.getTotalUnitsProduced(g->MANSELL) : 0;
            double totalmanurebought = g->MANBUY < (int)product_cat.size() ?Sector.getTotalUnitsProduced(g->MANBUY):0;
            double excessmanure = 0;
            double changeperha = g->CHANGEPERHA;  // Preisaenderungsrate
		...
		//MILK QUOTA
		if (i==g->GETQUOTA) { // lease milk quota

			double pricechange = 0;
            // milk produced in sector
            double totalmilk = Sector.getTotalUnitsProduced(g->MILK) * g->MILKPROD;
		...
		if (i==g->LETQUOTA) { // let milk quota
            	double pricechange = 0;
            	// milk produced in sector
            	double totalmilk = Sector.getTotalUnitsProduced(g->MILK) * g->MILKPROD;
            if (g->SWEDEN) {	
		...
		if ((i==g->SELLCALF_SUCK)||(i==g->BUYCALF_SUCK)) {
                // calculate excess suckler calf demand
                double
                excess_suck_calf=Sector.getTotalUnitsProduced(g->BUYCALF_SUCK)-Sector.getTotalUnitsProduced(g->SELLCALF_SUCK);
                ...
                if (excess_suck_calf>0) {
					...
					price_vector[i]*=(g->JOENKEPING)? 1.01 : 1.0;  // 1.0 Västerbotten
                } else {
                    //otherwise reduce price
					price_vector[i]*=(g->JOENKEPING) ? 0.99 : 1.0;  //1.0 Västerbotten
                }
		}

		// DAIRY CALF MARKET
        //Jump in the loop when we are at the calf activities
        if ((i==g->SELLCALF_DAIRY)||(i==g->BUYCALF_DAIRY)) {
                // calculate excess dairy calf demand
                double  excess_dairy_calf;
//DCX
				if (Sector.getTotalUnitsProduced(g->SELLCALF_DAIRY)>0)
					excess_dairy_calf=Sector.getTotalUnitsProduced(g->BUYCALF_DAIRY)/Sector.getTotalUnitsProduced(g->SELLCALF_DAIRY);
      ...
      //#### if (price_vector[i] < 0) {
	  if (old[i] < 0){ //####
			price_expectation_vector[i]=-(pow(fabs(old[i]),0.5)*pow(fabs(price_expectation_vector[i]),0.5)); // ####
...
RegMarketInfo::priceFunction(RegSectorResultsInfo& Sector,Evaluator* evaluator, int iteration ) {
    if (g->USE_VARIABLE_PRICE_CHANGE) {
        for (unsigned int i=0;i<product_cat.size();i++) {
            ...
            exp_price_change_vector[i]=evaluator->getVariable(getNameOfProduct(i)+"_price_change");
        }
    }
	//vector <double> old;            // ####changed 
