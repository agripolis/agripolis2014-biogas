...
//--------------
/*bool
    operator<(RegPlotInformationInfo& p1,RegPlotInformationInfo& p2 ) {
        return p1.costs() < p2.costs();
    }
//*/
//soil service
...
RegPlotInfo::setState(const int passState,RegFarmInfo* farm, int colour) {
    PA.state = passState;
  /*  if (PA.state == 2 || PA.state == 1 || PA.state == 3) {
        PA.plot_colour = colour;
    }
    if (PA.state == 0) {
        PA.plot_colour = clWhite;
    }
    if (PA.state == 4) {
        PA.plot_colour = clBlack;
        ;
    }
    //*/
    switch (PA.state) {
...
RegPlotInfo::identifyContiguousPlot(bool check_soiltype, bool check_farmnumber,bool check_previous_farmnumber,int farmnumberob,bool mark_update) {
    // Ich lege eine Liste und einen Stack an. Ich lasse die Schleife so lange laufen
    // bis keine möglichen Plots mehr vorhanden sind. Habe ich eine Plot geprüft so markiere ich eine boolsches Flag
    // um Dopplungen zu vermeiden. Deshalb benötige ich auch noch die List um alle markierten
    // Plt wieder zurückzusetzen (für spätere Überprüfungen)
...
RegPlotInformationInfo RegPlotInfo::calculateValue(RegPlotInfo* plot, RegFarmInfo* farm) {
    ...
    if (g->WEIGHTED_PLOT_SEARCH) {
        if (g->USE_TC_FRAMEWORK) {
            /*
            int neighbours = plot->identifyPreviousPlotsSameState(farm->getFarmId());
            double bonus=0;
            if (g->FIXED_BONUS)
