...
RegRegionInfo::setRentedPlot(RegPlotInfo* p,RegFarmInfo* f) {
    ...
    occupyPlot(p,f);
    int cl=g->MIN_CONTRACT_LENGTH+ randlong()%(g->MAX_CONTRACT_LENGTH-g->MIN_CONTRACT_LENGTH);
...
// finish up initialisation process of region
// sets plots which are neither arable nor grassland dead
...
RegRegionInfo::getRandomFreePlotOfType(int type) {
    ...
    do {
        test=false;
//        int randplot = (int)(((double)rand()/RAND_MAX)*(g->NO_COLS * g->NO_ROWS));
...
/*
// returns pointer to plot of type
RegPlotInfo*
RegRegionInfo::getRandomFreePlotOfType(int type)
{
    bool test=false;
